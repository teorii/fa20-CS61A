...
def swine_align(player_score, opponent_score):
    ...
    def gcd(plyrscr, opntscr):
        if plyrscr > opntscr:
            bigger, smaller = plyrscr, opntscr
        else:
            smaller, bigger = plyrscr, opntscr
        for divisor in range(smaller, 0, -1):
            if smaller % divisor == 0 and bigger % divisor == 0:
                return divisor
...
def pig_pass(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4b
    if -2 <= (player_score - opponent_score) <= -1:
        return True
    else:
        return False
    # END PROBLEM 4b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while running: 
        if who == 0:
            ...
            score0 += take_turn(num_rolls, score1, dice) # adding to score
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 1:
            ...
            tracking = score0
        if (tracking - last_score) > running_high:
            print(tracking - last_score, 'point(s)! The most yet for Player', who,)
            return announce_highest(who, tracking, tracking-last_score)
        return announce_highest(who, tracking, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    averages_of_rolls = []
    for num_of_rolls in range(1,11):
        average = make_averaged(roll_dice, trials_count)
        averages_of_rolls.append(average(num_of_rolls, dice))
    max(averages_of_rolls)
    return averages_of_rolls.index(max(averages_of_rolls)) + 1
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if 1 <= opponent_score - (score + free_bacon(opponent_score)) <= 2:
        return 0
