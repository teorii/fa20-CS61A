...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range = 0
        while check != beehive:
            if len(check.bees) > 0 and (self.min_range <= range <= self.max_range):
                return rANTdom_else_none(check.bees)
            else:
    ...
    def action(self, gamestate):
        ...
        if self.digesting == 0:
            if len(self.place.bees) > 0:
                import random
                bee_to_remove = random.choice(self.place.bees)
                self.eat_bee(bee_to_remove)
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that can farm bee lives on water."""
