...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(evaluated_operator)
    ...
    def apply(self, args, env):
        ...
        def convert(pair_list):
            if len(pair_list) == 0:
                ...
                return [pair_list.first] + convert(pair_list.rest)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        evaluated = scheme_eval(expressions.rest.first, env)
        env.define(target, evaluated)
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        lambda_form = do_lambda_form(expressions, env)
        lambda_form.formals = lambda_form.formals.rest
        env.define(target.first, lambda_form)
        return target.first
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings != nil:
        bfirst = bindings.first
        validate_form(bfirst, 2, 2)
        names = Pair(bfirst.first, names)
        values = Pair(scheme_eval(bfirst.rest.first, env), values)
