...
def balanced(m):
    ...
    if is_planet(m):
        ...
        lefty = length(left(m)) * total_weight(left_side)
        righty = length(right(m)) * total_weight(right_side)
        return lefty == righty and balanced(left_side) and balanced(right_side)
...
def preorder(t):
    ...
    for branch in branches(t):
        totalbranches = totalbranches + preorder(branch)
    return [label(t)] + totalbranches
