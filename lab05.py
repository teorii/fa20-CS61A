...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    a_distance = sqrt((get_lat(city_a)-lat)**2 + (get_lon(city_a)-lon)**2)
    b_distance = sqrt((lat-get_lat(city_b))**2 + (lon-get_lon(city_b))**2)
...
def sprout_leaves(t, leaves):
    ...
    branch = []
    for poke in branches(t):
        branch += [(sprout_leaves(poke, leaves))]
