...
def prune_min(t):
    ...
    """
    "*** YOUR CODE HERE ***"
    while len(t.branches) == 2:
        t.branches.remove(max(t.branches, key = lambda n : n.label))
    for n in t.branches:
        prune_min(n)
    ...
    def num_splits_helper(s, q):
        if len(s) == 0:
            if abs(q) <= d: 
                ...
                return 0
        first, rest = s[0], s[1:]
        return num_splits_helper(rest, q+first) + num_splits_helper(rest, q-first)
